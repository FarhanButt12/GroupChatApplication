...
export const AuthForm: React.FC<AuthFormProps> = ({
  ...
  const handleCustomClick = () => {
    if (googleBtnRef.current) {
      const btnElement = googleBtnRef.current.querySelector(
  ...
  return (
    <div className="min-h-screen glass-main flex items-center justify-center p-4 relative overflow-hidden select-none">
      <div className="w-full max-w-md glass-card p-8 rounded-3xl space-y-6 relative z-10 border border-slate-800 shadow-2xl">
        ...
        {/* Google OAuth Button at the bottom with dark background & invisible click overlay */}
        <div className="relative w-full">
          ...
          {/* Transparent overlay of Google's official interactive SDK target */}
          <div
            ref={googleBtnRef}
            className="absolute inset-0 opacity-0 overflow-hidden cursor-pointer pointer-events-auto"
          />
        </div>
      </div>
    </div>
