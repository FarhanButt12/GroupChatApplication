...
export const ChatRoom: React.FC<ChatRoomProps> = ({
  ...
  const fetchGroqSummary = async () => {
    ...
    try {
      const res = await fetch(`${apiUrl}/groups/${groupId}/ai-summary`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
        const data = await res.json();
        setAiSummaryText(data.summary || data.data?.summary || 'No daily summary has been generated for this group yet. Daily summaries run automatically via Redis background queues.');
      } else {
        const data = await res.json().catch(() => ({}));
        setAiSummaryText(data.message || 'No daily AI chat summary is available for this room yet.');
      }
    } catch (err) {
      setAiSummaryText('Unable to fetch AI summary from Groq AI service.');
    } finally {
    ...
    <div className="flex-1 flex flex-col h-full glass-main overflow-hidden relative">
      ...
      <header className="px-6 py-3.5 border-b border-slate-800/80 bg-slate-950/70 flex justify-between items-center shrink-0 backdrop-blur-md relative z-20">
        ...
        <div className="flex items-center gap-2">
          {/* Groq AI Summary Trigger Button */}
          <button
            onClick={fetchGroqSummary}
            className="flex items-center gap-1.5 text-[11px] px-3 py-1.5 rounded-xl font-black border border-indigo-500/40 bg-indigo-500/10 text-indigo-300 hover:bg-indigo-500/20 transition-all cursor-pointer shadow-md shadow-indigo-500/10 hover:scale-105"
          ...
          <button
            ...
            className="px-3 py-1.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-300 border border-rose-500/30 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5"
      ...
      {/* Groq AI Summary Modal */}
      {showAiModal && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl space-y-4 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <div className="flex items-center gap-2.5">
                <div className="w-9 h-9 rounded-xl bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 flex items-center justify-center text-lg font-black shadow-md">
                  🚀
                </div>
                <div>
                  <h3 className="font-black text-sm text-white flex items-center gap-2">
                    <span>Groq AI Daily Summary</span>
                    <span className="text-[10px] bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 px-2 py-0.5 rounded-full font-extrabold">
                      #{groupName}
                    </span>
                  </h3>
                  <p className="text-[11px] text-slate-400">Automated daily channel analysis powered by Groq LLM</p>
                </div>
              </div>
              <button
            ...
            <div className="py-2">
              {loadingAiSummary ? (
                <div className="py-8 flex flex-col items-center justify-center space-y-3">
                  ...
                  <p className="text-xs font-bold text-indigo-300 animate-pulse">Analyzing channel messages via Groq AI...</p>
                </div>
              ) : (
                <div className="p-4 bg-slate-950 border border-slate-800/80 rounded-2xl text-xs text-slate-300 leading-relaxed font-medium max-h-72 overflow-y-auto whitespace-pre-wrap">
                  {aiSummaryText}
                </div>
              )}
            </div>
            ...
            <div className="flex justify-end pt-1">
              <button
                onClick={() => setShowAiModal(false)}
                className="px-5 py-2 bg-slate-950 hover:bg-slate-800 text-slate-300 font-extrabold rounded-xl text-xs border border-slate-800 transition-all"
              >
                Close Summary
              </button>
            </div>
